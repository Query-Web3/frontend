...
<div class="max-w-7xl mx-auto">
  <Card class="mb-6">
    ...
    <form on:submit|preventDefault={handleSubmit} class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
      <div>
        ...
        <TableBody>
          {#each data as item}
            <TableBodyRow>
              ...
              <TableBodyCell>{item.yoy ? `${item.yoy.toFixed(2)}%` : '-'}</TableBodyCell>
              <TableBodyCell>{item.qoq ? `${item.qoq.toFixed(2)}%` : '-'}</TableBodyCell>
              <TableBodyCell>{item.txns.toLocaleString()}</TableBodyCell>
              <TableBodyCell>{item.txns_yoy ? `${item.txns_yoy.toFixed(2)}%` : '-'}</TableBodyCell>
            </TableBodyRow>
          {/each}
        </TableBody>
      </Table>
